...
onSnapshot(collection(db, "polls"), (snap) => {
  ...
  snap.forEach(docSnap => {
    ...
    const createdDate = poll.createdAt?.toDate?.() ? poll.createdAt.toDate().toLocaleDateString() : "Unknown date";
        ...
        <div style="margin: 0.5rem 0;">
          ${poll.options.map((option, index) => `
            <div style="display: flex; justify-content: space-between; padding: 0.25rem; background: #f8fafc; border-radius: 0.25rem; margin-bottom: 0.25rem;">
              <span>${option}</span>
              <span style="color: #6b7280; font-size: 0.8rem;">${poll.votes?.[index] || 0} votes</span>
            </div>
          `).join('')}
        </div>
